...
const Exam10 = ()=> {
    ...
    //- 함수를 호출할 때 이벤트 정보(e) 대신 아이템 정보(item)를 전달하여 처리하도록 처리
    const changeToEdit = (target)=>{
        //console.log(item);

        //아이템 변경
        const newItems = items.map(item=>{
            if(item.itemNo === target.itemNo){//target과 같은 번호의 상품 만큼은
                return{
                    ...item, //다른건 그대로 둬도
    ...
    //- data는 깨끗하게 정리
    //itemNO 관련 코드는 DB연결이 되면 사라질 코드이다
    const addItem = e=>{

        // const itemNo = 데이터가 없으면 ? 1 : 마지막 번호 + 1;
